...
def rotate_atoms_and_cell( atoms=None, target_z_direction=(0,0,1) ):
    """
    ...
    if ( np.allclose(orig_z,target_z_direction) ):
        return

    rot_axis = np.cross(target_z_direction,orig_z)
    rot_axis /= np.sqrt( np.sum(rot_axis**2) )
    angle = np.arccos( orig_z.dot(target_z_direction) )
    ...
    dotprod = new_z_direction.dot(target_z_direction)
